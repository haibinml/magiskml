...
#echo $curPath
#curl -s -o link $url
$curPath/核心/wget --no-check-certificate -O link $url
base64 -d link > vm
...
do
 ...
 remark=`echo $remarkfalse | sed 's/-//g' | sed 's/|//g' | sed 's/(//g' | sed 's/)//g' | sed 's/🇺🇲//g' | sed 's/🇸🇬//g' | sed 's/🇷🇺//g' | sed 's/🇰🇷//g' | sed 's/🇯🇵//g' | sed 's/🇮🇳//g' | sed 's/🇭🇰//g' | sed 's/🇬🇧//g' | sed 's/🇩🇪//g' | sed 's/🇨🇳//g' | sed 's/🇨🇦//g' | sed 's/🇧🇷//g' | sed 's/🇫🇷//g' | sed 's/🇪🇸//g' | sed 's/🌏//g' | sed 's/🇻🇳//g' | sed 's/🇹🇷//g' | sed 's/🇷🇴//g' | sed 's/🇳🇱//g' | sed 's/🇮🇹//g' | sed 's/🇮🇩//g' | sed 's/|//g' | sed 's/>//g' | sed 's/丨//g'`
 if [ $net = "tcp" ]
     then
       method="GET"
     else
...
. ./config.ini
if [ "$sjd" = "1" ]; then
grep -q "*,*" config.ini || sed -i "/file=/cfile=$filename," config.ini
grep -q "*," config.ini || sed -i "/file/cfile=$jdm$filename" config.ini
else
sed -i "/file=/cfile=$filename," config.ini
fi
./开启.sh
sed -i "/jdm=/cjdm=$filename," config.ini
echo -e "
- 节点成功更换"
...
rm -f ${0%/*}/*.bak
echo 扫码加QQ群
echo "https://jq.qq.com/?_wv=1027&k=DV5pbLo7" | curl -F-=\<- qrenco.de
